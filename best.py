...
hold_time_stats = valid_keys[valid_keys.Hand.isin(['L', 'R', 'S'])].groupby(['user_id', 'Hand'])['Hold time'].agg([np.mean, np.median, np.max, np.min, np.var, iqr, skew, kurtosis])
latency_time_stats = valid_keys[valid_keys.Direction.isin(['LL', 'LR', 'RR', 'RL'])].groupby(['user_id', 'Direction'])['Latency time'].agg([np.mean, np.median, np.max, np.min, np.var, iqr, skew, kurtosis])
...
features = [1] + list(range(5,64))
X = final_user_dataset.iloc[:, features]
...
columns_optimal = ['Tremors', 'Sided_None', 'mean_L_hold_time', 'mean_S_hold_time',
...
X_op = X.loc[:, columns_optimal].values

# Create train and test sets
from sklearn.model_selection import train_test_split
X_train, X_test, y_train, y_test = train_test_split(X_op, y, test_size=2/5, random_state=0, stratify=y)
